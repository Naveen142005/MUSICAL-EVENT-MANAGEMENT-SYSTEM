...
from app.utils.common import commit, create_error, insert_data, update_data, validate_image_ex
from app.services.payment import payment_service


router = APIRouter()
details_router = APIRouter(tags=["Event Details"])

organizer_create = APIRouter(prefix='/events', tags=["Events"])

@organizer_create.post('/create_new_event', response_model=EventResponse)
async def create_new_event(event_data:EventBase,  db:Session = Depends(db.get_db), current_user: dict = Depends(role_requires("Organizer"))):
    ...
    if (event_data.event_date- date.today()).days >= 365:
        raise HTTPException(status_code=404, detail="Event can't book by more than a year")
    user_id = current_user["id"]
    return await event.create_new_event(event_data, user_id,db)
    

@organizer_create.patch("/update_banner")
def update_banner( 
...
@organizer_create.get("/events/{event_id}")
async def get_event_by_id(event_id: int, db: Session = Depends(db.get_db), current_user: dict = Depends(role_requires("Organizer", "Admin"))):
...
@details_router.get("/audience/search")
def search_events(
...
organizer_bookings = APIRouter(tags=["Organizer_Bookings"])
@organizer_bookings.get('/my_bookings/upcoming')
...
@organizer_bookings.get('/my_bookings/past')
...
@organizer_bookings.get('/my_bookings/pending')
...
@organizer_bookings.get('/my_bookings/ongoing')
...
@organizer_bookings.post('/pay_pending_amount/{event_id}')
...
reschedule_router = APIRouter(prefix='/events', tags=["Event Rescedule"])

@reschedule_router.get('/facilities')
...
@reschedule_router.get('/get_reschedule_dates')
...
@reschedule_router.patch('/reschedule_event')
...
@organizer_create.post("/event_cancel/{event_id}")
...
@details_router.get("/admin/booked-events")
...
ticket_route = APIRouter(tags=['Tickets & Invoice'])

@ticket_route.get('/view_ticket_details/{event_id}')
...
@ticket_route.get("/download/{event_id}")
...
router.include_router(organizer_create)
router.include_router(organizer_bookings)
router.include_router(ticket_route)
