...
def get_available_tickets(event_id: int, db: Session = Depends(db.get_db), current_user: dict = Depends(role_requires("Audience"))):
    ...
    if event_id not in booking_service.get_pubilc_event_id(db):
        raise HTTPException(status_code=404, detail="This event is not available for u to book.")
...
@router.get('/refund_status/{refund_id}')
def check_refund(refund_id: int, db:Session = Depends(db.get_db)):
    print(refund_id)
    data = get_row(db, Refund, id= refund_id)
    return {
        "id": data.id,
