...
from app.utils.common import get_image_url, get_row
...
class UserService:
    ...
    def update_profile_image(self, db: Session, user_id: int, image_url: str):
        ...
        if details and details.profile_image:
            old_image = details.profile_image.replace("\\", "/")

            # --- Fix for leading slash issue ---
            if old_image.startswith("/"):
                old_image = old_image[1:]  # remove the leading '/'

            base_dir = os.path.abspath(os.path.dirname(__file__))
            project_root = os.path.abspath(os.path.join(base_dir, "../.."))  # go 2 levels up
            abs_old_path = os.path.join(project_root, old_image)
            abs_old_path = os.path.abspath(abs_old_path)

            if os.path.exists(abs_old_path):
                os.remove(abs_old_path)
                print(f"✅ Removed old image: {abs_old_path}")
            else:
                print(f"⚠️ Old image not found: {abs_old_path}")
