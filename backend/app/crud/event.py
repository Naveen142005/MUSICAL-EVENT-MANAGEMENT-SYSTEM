...
class EventCRUD:
    ...
    # Get facility details from MongoDB
        ...
        response =  event_service.build_event_response(event_data, event.id, user_id=user_id, payment_id=payment.id, total_amount= total_amount, paid_amount=paying_amount)
    ...
    async def pay_pending_amount(self, event_id: int, user_id: int, payment_mode, db: Session):
        ...
        if user_data.id != event_data.user_id:
            raise HTTPException (status_code=404, detail=f"The event {event_id} is not belongs to {user_id}. Its belongs to {event_data.user_id}")
    ...
    # Get facility details from MongoDB
        facility_doc = await col.find_one({"event_id": event_id})
        ...
        if event_data.ticket_enabled:
            ...
            #         "count": event_data.silver_ticket_count
            #     })
            ticket_data = get_rows(db, Tickets, event_id = event_id)
            for i in ticket_data:
                tickets.append({
                    "type": i.ticket_type_name,
                    "price": i.price,
                    "count": (i.available_counts + i.booked_ticket)
                })
        
        # Generate Invoice
        from app.utils.invoice_generator import generate_event_invoice
        print(facility_doc.get('snacks'))
        print(facility_doc.get('snacks').get('count'))
        invoice_path = generate_event_invoice(
            invoice_number=f"INV{event_id:06d}",
            event_id=event_id,
            event_name=event_data.name,
            event_date=event_data.event_date.strftime('%d %b %Y'),
            slot=event_data.slot,
            customer_name=user_data.name,
            customer_email=user_data.email,
            customer_phone=str(user_data.phone),
            ...
            decoration_data=facility_doc.get('decoration') if facility_doc else None,
            snacks_data=facility_doc.get('snacks') if facility_doc else None,
            snacks_count=facility_doc.get('snacks').get('count') or 0,
            ticket_details=tickets if tickets else None,
            total_amount=event_data.total_amount,
            paid_amount=event_data.total_amount,
            payment_type=payment_mode
        )
        
        
        return "SUCCESSFULLY AMOUNT PAID"
