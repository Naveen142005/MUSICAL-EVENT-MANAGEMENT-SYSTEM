...
class BookingCRUD:
    async def create_booking(_, db: Session, booking_data: BookingCreate,user_id):
        try:
            edata = get_row(db, Event, id = booking_data.event_id)
            if not edata:
                raise HTTPException(status_code=404, message ='Event data not Found')
            print(type(edata.ticket_open_date))
            
            if edata.ticket_open_date > date.today():
                raise HTTPException(
                    status_code=404,
                    detail=f"Ticket booking will open at {edata.ticket_open_date}"
                )
            merged_tickets = {}
            ...
            for td in ticket_details:
                ...
                up_row = db.query(Tickets).filter(
                    and_ (
                        Tickets.event_id == booking_data.event_id,
                        func.lower(Tickets.ticket_type_name) == func.lower(td["ticket_type"]),
                        Tickets.available_counts >= td["quantity"]  
                    )
                ).update({
                    Tickets.available_counts: Tickets.available_counts - td["quantity"],
                    Tickets.booked_ticket: Tickets.booked_ticket + td["quantity"]
                ...
                if up_row == 0:
                    raise HTTPException(status_code=400, detail=f"Tickets for {td['ticket_type']} sold out or insufficient.")
